...
class PromptImprovementManager(BaseAgent):
    ...
    async def run(self, question: str, context: dict | None = None) -> AgentResponse:
        state = context or {}
        try:
            prompt = self._load_prompt("managers/learning/prompt_improvement_manager")
        except Exception:
            prompt = "You are the Prompt Improvement Manager. Respond with JSON: {status, summary, metrics, recommendations}."
        messages = [
