...
class MembershipManager(BaseAgent):
    ...
    async def run(self, question: str, context: dict | None = None) -> AgentResponse:
        state = context or {}
        try:
            prompt = self._load_prompt("managers/cro/membership_manager")
        except Exception:
            prompt = "You are the Membership Manager. Respond with JSON: {status, summary, metrics, recommendations}."
        messages = [
